(function(window, document) {
    ...
    var clength = modelsToLoad.length - 2;
    var step = (2 * Math.PI) / clength;
    var angle = 0;
    var circle = {
      width: 50,
    ...
    modelsToLoad.forEach(function (model, i) {
        ...
        mtlLoader.load(model.name+'.obj.mtl', function (materials) {
            materials.preload();
            objLoader.setMaterials(materials);

            objLoader.load(model.name+'.obj', function (object) {
                if (model.circle) {
                    object.scale.set(10, 10, 10);
                    object.position.y = -10;
                    object.position.x = (circle.width / clength) + (circle.radius * Math.cos(angle));
                    object.position.z = (circle.depth / clength) + (circle.radius * Math.sin(angle));
                    angle += step;
                } else {
                    object.scale.set(10, 10, 10);
                ...
                object.lookAt(new T.Vector3(camera.position.x, 0, camera.position.z));
                scene.add(object);
            });
        });
    });
